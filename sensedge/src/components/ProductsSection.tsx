          ...
          <div className="hidden md:flex flex-col gap-1 w-52 flex-shrink-0">
            {gameCategories.map((game) => (
              <button
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs font-medium transition-all ${activeGame === game.id
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {gameCategories.map((game) => (
                <button
                  key={game.id}
                  onClick={() => setActiveGame(game.id)}
                  className={`flex-shrink-0 flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all ${activeGame === game.id
